...
class UserRestControllerTest {
    ...
    @Sql(statements = "DELETE FROM users WHERE username = 'newuser';")
    void testCreateUser_Success() throws Exception {
        ...
        assertResult.bodyJson().extractingPath("$.id").asString().isNotEmpty();
    ...
    @Test
    @WithMockUser
    @DisplayName("バリデーションエラーがある場合は400エラーが返されること - ユーザー名が短すぎる")
    void testCreateUser_ValidationError_UsernameTooShort() throws Exception {
        String requestBody = """
                {
                    "username": "ab",
    ...
    @Test
    @WithMockUser
    @DisplayName("バリデーションエラーがある場合は400エラーが返されること - メールアドレスが無効")
    ...
    @Test
    @WithMockUser
    @DisplayName("バリデーションエラーがある場合は400エラーが返されること - パスワードが短すぎる")
    void testCreateUser_ValidationError_PasswordTooShort() throws Exception {
        String requestBody = """
                {
                    "username": "testuser",
                    "email": "test@example.com",
    ...
    @Test
    @WithMockUser
    @DisplayName("既存ユーザーを更新できること")
    @Sql(statements = {
            """
            INSERT INTO users (id, username, email, password_hash, enabled)
            VALUES ('a50e8400-e29b-41d4-a716-446655440001', 'oldusername', 'old@example.com', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy', true)
            ON CONFLICT DO NOTHING;
            """
    })
    void testUpdateUser_Success() throws Exception {
        ...
        assertResult.bodyJson().extractingPath("$.id").asString().isEqualTo(userId);
        assertResult.bodyJson().extractingPath("$.username").asString().isEqualTo("updatedusername");
    ...
    @Test
    @WithMockUser
    @DisplayName("部分更新をサポートすること - ユーザー名のみ更新")
    @Sql(statements = {
            """
            INSERT INTO users (id, username, email, password_hash, enabled)
            VALUES ('b50e8400-e29b-41d4-a716-446655440001', 'oldusername', 'old@example.com', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy', true)
            ON CONFLICT DO NOTHING;
            """
    })
    ...
    @Test
    @WithMockUser
    @DisplayName("更新時のバリデーションエラーがある場合は400エラーが返されること")
    @Sql(statements = {
            """
            INSERT INTO users (id, username, email, password_hash, enabled)
            VALUES ('c50e8400-e29b-41d4-a716-446655440001', 'testuser', 'test@example.com', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy', true)
            ON CONFLICT DO NOTHING;
            """
    })
